...
Bool ephyrScreenInitialize(KdScreenInfo *screen) {
  ...
  if (screen->fb.depth && screen->fb.depth != hostx_get_depth()) {
    if (screen->fb.depth < hostx_get_depth() &&
      ...
      ErrorF("\nXephyr: requested screen depth not supported, setting to match "
             "hosts.\n");
  }
  ...
  if (screen->fb.depth <= 8) {
    if (EphyrWantGrayScale)
      ...
      screen->fb.visuals =
          ((1 << StaticGray) | (1 << GrayScale) | (1 << StaticColor) |
           (1 << PseudoColor) | (1 << TrueColor) | (1 << DirectColor));
...
void *ephyrWindowLinear(ScreenPtr pScreen, CARD32 row, CARD32 offset, int mode,
...
Bool ephyrMapFramebuffer(KdScreenInfo *screen) {
  ...
  EPHYR_LOG("screen->width: %d, screen->height: %d index=%d", screen->width,
            screen->height, screen->mynum);
  ...
  priv->base = hostx_screen_init(
      screen, screen->x, screen->y, screen->width, screen->height,
      buffer_height, &priv->bytes_per_line, &screen->fb.bitsPerPixel);
  ...
  if ((scrpriv->randr & RR_Rotate_0) && !(scrpriv->randr & RR_Reflect_All)) {
    ...
    KdShadowFbAlloc(screen, scrpriv->randr & (RR_Rotate_90 | RR_Rotate_270));
  }
...
static void ephyrInternalDamageRedisplay(ScreenPtr pScreen) {
  KdScreenPriv(pScreen);
  KdScreenInfo *screen = pScreenPriv->screen;
  EphyrScrPriv *scrpriv = screen->driver;
  RegionPtr pRegion;
  ...
  if (RegionNotEmpty(pRegion)) {
    ...
    if (ephyr_glamor) {
      ...
      while (nbox--) {
        hostx_paint_rect(screen, pbox->x1, pbox->y1, pbox->x1, pbox->y1,
...
static void ephyrXcbProcessEvents(Bool queued_only);

static Bool ephyrEventWorkProc(ClientPtr client, void *closure) {
  ephyrXcbProcessEvents(TRUE);
  return TRUE;
}

static void ephyrScreenBlockHandler(ScreenPtr pScreen, void *timeout) {
  KdScreenPriv(pScreen);
  KdScreenInfo *screen = pScreenPriv->screen;
  EphyrScrPriv *scrpriv = screen->driver;
...
Bool ephyrSetInternalDamage(ScreenPtr pScreen) {
  ...
  scrpriv->pDamage = DamageCreate((DamageReportFunc)0, (DamageDestroyFunc)0,
...
Bool ephyrRandRGetInfo(ScreenPtr pScreen, Rotation *rotations) {
  ...
  struct {
    int width, height;
  } sizes[] = {{1600, 1200}, {1400, 1050}, {1280, 960}, {1280, 1024},
               {1152, 864},  {1024, 768},  {832, 624},  {800, 600},
               {720, 400},   {480, 640},   {640, 480},  {640, 400},
               {320, 240},   {240, 320},   {160, 160},  {0, 0}};
  ...
  *rotations = RR_Rotate_All | RR_Reflect_All;
  ...
  if (!hostx_want_preexisting_window(screen) &&
    ...
    while (sizes[n].width != 0 && sizes[n].height != 0) {
      RRRegisterSize(pScreen, sizes[n].width, sizes[n].height,
...
Bool ephyrRandRSetConfig(ScreenPtr pScreen, Rotation randr, int rate,
                         RRScreenSizePtr pSize) {
  KdScreenPriv(pScreen);
  KdScreenInfo *screen = pScreenPriv->screen;
  EphyrScrPriv *scrpriv = screen->driver;
...
#ifdef GLAMOR
  ephyr_glamor_set_window_size(scrpriv->glamor, scrpriv->win_width,
                               scrpriv->win_height);
#endif
  ...
  if (scrpriv->shadow) {
    if (!KdShadowSet(screen->pScreen, scrpriv->randr, ephyrShadowUpdate,
                     ephyrWindowLinear))
      goto bail4;
  } else {
#ifdef GLAMOR
   ...
   */
  (*pScreen->ModifyPixmapHeader)(fbGetScreenPixmap(pScreen), pScreen->width,
                                 pScreen->height, screen->fb.depth,
...
static Bool ephyrResizeScreen(ScreenPtr pScreen, int newwidth, int newheight) {
  KdScreenPriv(pScreen);
  KdScreenInfo *screen = pScreenPriv->screen;
  RRScreenSize size = {0};
  Bool ret;
...
Bool ephyrInitScreen(ScreenPtr pScreen) {
  ...
  if (EphyrWantNoHostGrab) {
    ...
    hostx_set_win_title(screen, "(ctrl+esc grabs mouse and keyboard)");
  }
...
Bool ephyrCreateResources(ScreenPtr pScreen) {
  ...
  EPHYR_LOG("mark pScreen=%p mynum=%d shadow=%d", pScreen, pScreen->myNum,
            scrpriv->shadow);

  if (scrpriv->shadow)
    return KdShadowSet(pScreen, scrpriv->randr, ephyrShadowUpdate,
                       ephyrWindowLinear);
  else {
#ifdef GLAMOR
    if (ephyr_glamor) {
...
void ephyrCloseScreen(ScreenPtr pScreen) { ephyrUnsetInternalDamage(pScreen); }

/*
 * Port of Mark McLoughlin's Xnest fix for focus in + modifier bug.
 * See https://bugs.freedesktop.org/show_bug.cgi?id=3030
 */
void ephyrUpdateModifierState(unsigned int state) {
...
static Bool ephyrCursorOffScreen(ScreenPtr *ppScreen, int *x, int *y) {
  return FALSE;
}

static void ephyrCrossScreen(ScreenPtr pScreen, Bool entering) {}

ScreenPtr ephyrCursorScreen; /* screen containing the cursor */

static void ephyrWarpCursor(DeviceIntPtr pDev, ScreenPtr pScreen, int x,
                            int y) {
  input_lock();
...
static KdScreenInfo *screen_from_window(Window w) {
  int i = 0;
  ...
  for (i = 0; i < screenInfo.numScreens; i++) {
    ...
    if (scrpriv->win == w || scrpriv->peer_win == w ||
...
static void ephyrProcessErrorEvent(xcb_generic_event_t *xev) {
  ...
  FatalError("X11 error\n"
             ...
             "Error value: %u\n",
             e->error_code, e->sequence, e->major_code, e->minor_code,
             e->resource_id);
}

static void ephyrProcessExpose(xcb_generic_event_t *xev) {
  ...
  if (scrpriv) {
    hostx_paint_rect(scrpriv->screen, 0, 0, 0, 0, scrpriv->win_width,
                     scrpriv->win_height);
  } else {
...
static void ephyrProcessMouseMotion(xcb_generic_event_t *xev) {
  ...
  if (ephyrCursorScreen != screen->pScreen) {
    ...
    ephyrWarpCursor(inputInfo.pointer, screen->pScreen, motion->event_x,
                    motion->event_y);
  } else {
    int x = 0, y = 0;
...
static void ephyrProcessButtonPress(xcb_generic_event_t *xev) {
...
static void ephyrProcessButtonRelease(xcb_generic_event_t *xev) {
  xcb_button_press_event_t *button = (xcb_button_press_event_t *)xev;
...
/* Xephyr wants ctrl+shift to grab the window, but that conflicts with
   ...
   effort.
 */
static int ephyrUpdateGrabModifierState(int state) {
  static int mod1_was_down = 0;

  if ((state & (XCB_MOD_MASK_CONTROL | XCB_MOD_MASK_SHIFT | XCB_MOD_MASK_1)) ==
      0)
    mod1_was_down = 0;
...
static void ephyrProcessKeyPress(xcb_generic_event_t *xev) {
  xcb_key_press_event_t *key = (xcb_key_press_event_t *)xev;

  if (!ephyrKbd || !((EphyrKbdPrivate *)ephyrKbd->driverPrivate)->enabled) {
    return;
  }
...
static void ephyrProcessKeyRelease(xcb_generic_event_t *xev) {
  ...
  int keypressed = xcb_key_symbols_get_keysym(keysyms, key->detail, 0);
  int ctrlpressed = key->state & XCB_MOD_MASK_CONTROL;
  if (!EphyrWantNoHostGrab && (keypressed == XK_Escape) && (ctrlpressed)) {
    ...
    if (grabbed_screen != -1) {
      ...
      xcb_grab_keyboard_cookie_t kbgrabc =
          xcb_grab_keyboard(conn, TRUE, scrpriv->win, XCB_TIME_CURRENT_TIME,
                            XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC);
      xcb_grab_keyboard_reply_t *kbgrabr;
      xcb_grab_pointer_cookie_t pgrabc = xcb_grab_pointer(
          conn, TRUE, scrpriv->win, 0, XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC,
          scrpriv->win, XCB_NONE, XCB_TIME_CURRENT_TIME);
      ...
      if (!kbgrabr || kbgrabr->status != XCB_GRAB_STATUS_SUCCESS) {
        ...
        if (!pgrabr || pgrabr->status != XCB_GRAB_STATUS_SUCCESS) {
          ...
          hostx_set_win_title(screen, "(ctrl+esc releases mouse and keyboard)");
        }
      }
    }
  }
...
static void ephyrProcessConfigureNotify(xcb_generic_event_t *xev) {
  xcb_configure_notify_event_t *configure = (xcb_configure_notify_event_t *)xev;
  KdScreenInfo *screen = screen_from_window(configure->window);
  EphyrScrPriv *scrpriv = screen->driver;

  if (!scrpriv || (scrpriv->win_pre_existing == None && !EphyrWantResize)) {
    return;
  }
...
static void ephyrXcbNotify(int fd, int ready, void *data) {
...
void ephyrPutColors(ScreenPtr pScreen, int n, xColorItem *pdefs) {
  ...
  while (n--) {
    ...
    hostx_set_cmap_entry(pScreen, p, pdefs->red >> 8, pdefs->green >> 8,
                         pdefs->blue >> 8);
    pdefs++;
  }
  if (scrpriv->pDamage) {
...
static Status MouseInit(KdPointerInfo *pi) {
  pi->driverPrivate =
      (EphyrPointerPrivate *)calloc(sizeof(EphyrPointerPrivate), 1);
...
static void MouseFini(KdPointerInfo *pi) {
...
KdPointerDriver EphyrMouseDriver = {
    "ephyr", MouseInit, MouseEnable, MouseDisable, MouseFini, NULL,
};

/* Keyboard */

static Status EphyrKeyboardInit(KdKeyboardInfo *ki) {
  KeySymsRec keySyms;
  CARD8 modmap[MAP_LENGTH];
  XkbControlsRec controls;

  ki->driverPrivate = (EphyrKbdPrivate *)calloc(sizeof(EphyrKbdPrivate), 1);
...
static Status EphyrKeyboardEnable(KdKeyboardInfo *ki) {
  ((EphyrKbdPrivate *)ki->driverPrivate)->enabled = TRUE;

  return Success;
}

static void EphyrKeyboardDisable(KdKeyboardInfo *ki) {
  ((EphyrKbdPrivate *)ki->driverPrivate)->enabled = FALSE;
}

static void EphyrKeyboardFini(KdKeyboardInfo *ki) {
  free(ki->driverPrivate);
  ephyrKbd = NULL;
  return;
}

static void EphyrKeyboardLeds(KdKeyboardInfo *ki, int leds) {}
